...
export interface Product {
    ...
    active: boolean;
    /**
     * The product's name, meant to be displayable to the customer. Whenever this product is sold via a subscription,
    ...
    chatBotConfig: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>;
...
export interface Subscription {
    ...
    chatBotConfig: FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>;
    /**
     * Array of price references. If you prvoide multiple recurring prices to the checkout session via the `line_items`
     * parameter, this array will hold the references for all recurring prices for this subscription. `price ===
     * prices[0]`.
     */
    prices: Array<
     ...
     * `cancel_at_period_end`, `canceled_at` will still reflect the date of the initial cancellation request, not the
